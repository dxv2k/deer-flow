...
class BackgroundResearchProcessor:
    ...
    async def process_research_task(self, task_id: str) -> Dict[str, Any]:
        """
        ...
        with self.langfuse_client.start_as_current_span(
            ...
            try:
                ...
                await self.task_manager.update_task_status(
                    task_id, ResearchStatus.RUNNING
                )
                ...
                try:
                    ...
                    await self.task_manager.update_task_status(
                        task_id, ResearchStatus.FAILED, error=error_msg
                    )
                ...
                try:
                    ...
                    if not isinstance(result, dict):
                        raise ValueError(
                            f"Workflow returned unexpected type: {type(result)}, expected dict"
                        )

                    final_report = result.get("final_report", "")
                    if not final_report:
                        logger.warning(
                            f"Task {task_id}: No final_report in result. Available keys: {list(result.keys())}"
                        )
                    ...
                    logger.info(
                        f"Task {task_id}: Extracted final_report length: {len(final_report)}, sources: {sources_count}"
                    )
                ...
                try:
                    ...
                    logger.info(
                        f"Task {task_id}: Generating summary and insights from final report"
                    )

                    with self.langfuse_client.start_as_current_span(
                        name="summarize-research-result"
                    ) as span:
                        span.update(
                            input={
                        ...
                        if summarization_result.get("success", False):
                            ...
                            logger.info(
                                f"Task {task_id}: Successfully generated summary ({len(summary) if summary else 0} chars) and {len(insights)} insights"
                            )
                            span.update(
                                output={
                                    "summary": summary,
                            ...
                            error_msg = summarization_result.get(
                                "error", "Unknown summarization error"
                            )
                            logger.warning(
                                f"Task {task_id}: Summarization failed but continuing: {error_msg}"
                            )
                            span.update(
                                output={
                                    "error": error_msg,
                    ...
                    logger.warning(
                        f"Task {task_id}: Summarization failed but continuing research completion: {str(summarization_error)}"
                    )
                ...
                try:
                    ...
                    if not update_success:
                        logger.error(
                            f"Failed to update task {task_id} status to completed"
                        )
                ...
                logger.info(
                    f"Research completed for task {task_id} in {execution_time:.2f}s"
                )
                ...
                if task.submit_to_mktagent and self.mktagent_client:
                    try:
                        ...
                        logger.error(
                            f"MktAgent submission failed for task {task_id}: {str(mktagent_error)}"
                        )
                        logger.error(f"Full traceback: {traceback.format_exc()}")
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to update task {task_id} status after error: {str(update_error)}"
                    )
                failed_output = {
                    "task_id": task_id,
                    "status": "failed",
    ...
    async def _execute_research_workflow(self, task) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Starting workflow execution with config: {config['configurable']}"
            )
            ...
            async for state in graph.astream(
                ...
                final_state = state
                logger.debug(
                    f"Workflow state update: {list(state.keys()) if isinstance(state, dict) else type(state)}"
                )
            ...
            logger.info(
                f"Workflow completed. Final state keys: {list(final_state.keys()) if isinstance(final_state, dict) else 'Not a dict'}"
            )

            # Validate the final state
            if not isinstance(final_state, dict):
                raise ValueError(
                    f"Final state is not a dictionary: {type(final_state)}"
                )
            ...
            logger.info(
                f"Observations count: {len(observations) if observations else 0}"
            )
        ...
        try:
            ...
            if mktagent_content_id:
                try:
                    ...
                    if update_success:
                        logger.info(
                            f"Task {task_id} submitted to mktagent as content {mktagent_content_id}"
                        )
                    else:
                        logger.error(
                            f"Failed to update task {task_id} with mktagent content ID"
                        )
                except Exception as update_error:
                    logger.error(
                        f"Failed to update task {task_id} status after mktagent submission: {str(update_error)}"
                    )
                    logger.error(f"Full traceback: {traceback.format_exc()}")
            else:
    ...
    async def submit_task_async(self, task_id: str):
        ...
        try:
            ...
            logger.info(
                f"Background processing completed for task {task_id}: {result.get('status', 'unknown')}"
            )
        except Exception as e:
            ...
            try:
                ...
                logger.error(
                    f"Failed to update task {task_id} status after background error: {str(update_error)}"
                )
